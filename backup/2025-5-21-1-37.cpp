...
class Book{
    public:
        //构造函数
        Book(string t,string a,string i,string pre,string d,double pri,int q,int s,int f):
        title(t),author(a),isbn(i),press(pre),date(d),price(pri),quantity(q),stock(s),isfree(f){}    
        ...
        //整合数据
        string savestr(){
            return title+','+author+','+isbn+','+press+','+date+','+to_string(price)+','+to_string(quantity)+','+to_string(stock)+','+to_string(isfree);
        }

    private:
        string title,author,isbn,press,date;//字符串类型：书名、编号、作者、出版社、出版日期
        double price;//浮点类型：价格
        int quantity,stock,isfree;//整型：数量、库存、是否可被借阅
};
...
void Booklist_init(vector<Book>& book_list,string csv){
    ...
    while(getline(file,line)){
        ...
        string item[9];//9项数据使用数组存储    
        int i=0;
        while(getline(ss,item[i],',')){i++;}
        book_list.emplace_back(item[0],item[1],item[2],item[3],item[4],stod(item[5]),stoi(item[6]),stoi(item[7]),stoi(item[8]));
    }
    file.close();
}
...
//读取可被借阅图书内容
void Booklist_info_iffree(vector<Book>& book_list){
    int i=1;
    for(auto& book:book_list){
        o(i,TAB);
        book.showinfo_iffree();
        i++;
    }
}
...
int borrow_menu(vector<Book>& book_list){
    ...
    Booklist_info_iffree(book_list);
    int totnum=book_list.size();
    cout<<"总计"<<totnum<<"本可被借阅的书籍。"<<endl;
    int cid;
