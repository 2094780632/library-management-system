...
//一个制表符长短可控的内联输出函数,align true for right,default for left
void o(const string &text,int t,bool align){
    if(align){
        cout<<setw(t)<<text;
    }else{
        cout<<text<<setw(t);
...
void o(const double &d,int a,int t,bool align){
    if(align){
        cout<<fixed<<setprecision(a)<<setw(t)<<d;
    }else{
        cout<<d<<fixed<<setprecision(a)<<setw(t);
    }
}
//整型映射到普通输出
inline void o(const int a){o(to_string(a));}
inline void o(const int a,int t,bool align){o(to_string(a),TAB,align);}
//输出并暂停
...
class Book{
    public:
        ...
        void showinfo(){
            ...
            o(date,TAB,true);
            o(price,2,TAB,true);
            o(to_string(quantity),TAB,true);
            o(to_string(stock),TAB,true);
        ...
        //简要信息展示
        void briefinfo(){
            o(title,BRIEFTAB,true);
            //o(author,TAB);
            o(to_string(stock),BRIEFTAB,true);
            o("");
        }
...
void Booklist_briefheading(bool isid=false){
    if(isid){o("序号",BRIEFTAB,false);}
    o("书名",BRIEFTAB,false);
    //o("作者",TAB);
    o("可借数量",BRIEFTAB,false);
    o("");
}
...
class User{
    public:
        ...
        void showinfo(){
            ...
            o(id,TAB,true);
            o(status,TAB,true);
            o(vetostr(borrowed_list),TAB,true);
            o("");
        }
...
MenuState borrow_menu(vector<Book>& book_list,vector<User>& user_list){
    ...
    for(size_t i=0;i<book_list.size();++i){
        if(book_list[i].isfree()){
            //序号
            o(to_string(freenum+1),TAB,true);
